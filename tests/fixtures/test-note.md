...
The trigger syntax is `\#RRGGBB` (backslash-escaped hex code). Unescaped
hex codes like `#ff0000` remain normal Obsidian tags — only `\#ff0000`
produces a swatch.

---

## 1 · Basic inline hex (digit-leading)
...
## 2 · Letter-leading hex

The brand colours are \#ff0000 and \#aabbcc and \#ffffff.

**Expected:** Same as scenario 1 — swatches shown, not tag links.
...
## 9 · Unescaped hex codes — remain as tags (no swatch)

These are normal unescaped hex codes — they become Obsidian tags, not swatches:

- #ff0000
- #1a73e8
- #123456

**Expected:** Obsidian renders these as tag links. No swatches.

---

## 10 · Invalid / near-miss hex codes — should NOT render

These must all be ignored (even when escaped):
...
## 14 · Cursor-aware editing (Live Preview only)

Place your cursor inside a `\#RRGGBB` token in Live Preview.

**Expected:** The swatch and styled span collapse back to the raw `\#RRGGBB`
text while the cursor is inside it, then render again once you move away.
